...
public class US18_IlacErismeVeRandevuOlusturmaTesti extends TestBaseRapor {
    ...
    public void tc04_MedicineDetailPageTest() {
        ...
        if (!contentElements.isEmpty()) {
            extentTest
                    .pass("İlacın açıklama bilgisi görünür (Bulunan paragraf sayısı: " + contentElements.size() + ")");
        } else {
        ...
        if (pageContent.length() > 100) {
            extentTest.pass("✅ Kullanım talimatları ve detaylı bilgiler görünür (İçerik uzunluğu: "
                    + pageContent.length() + " karakter)");
        } else {
        ...
        try {
            ...
            try {
                if (appointmentForm.successMessage.isDisplayed()) {
                    ...
                    extentTest.fail(
                            "❌ STEP 2 FAILED: BUG! Appointment Booking butonu görünür ve aktif, ancak form validasyonu çalışmıyor");
                    extentTest.fail(
                            "❌ STEP 3 FAILED: BUG! Boş formla randevu oluşturuldu! Form alanları boş olmasına rağmen sistem randevuyu kabul etti");
                    extentTest.fail(
                            "❌ STEP 4 FAILED: BUG! Randevu formu açıldı ancak zorunlu alanlar (tarih, saat, telefon, departman, doktor, mesaj) doldurulmadan işlem tamamlandı");
                    extentTest.fail(
                            "❌ STEP 5 FAILED: BUG! Tarih (30/01/2026), telefon (geçerli format), departman, doktor ve mesaj alanları boş olmasına rağmen randevu başarıyla kaydedildi");
                    extentTest.fail("❌ STEP 6 FAILED: BUG! Randevu başarılı mesajı gösterildi: '" + successText
                            + "' - Form validasyonu tamamen devre dışı!");
                ...
                if (pageText.contains("Congratulations") || pageText.contains("success")
                        || pageText.contains("Success")) {
                    extentTest.fail(
                            "❌ STEP 2-6 FAILED: BUG! Success message butonun locator'ı bulunamadı ama sayfada 'Congratulations' mesajı var!");
                    extentTest.fail("❌ Boş formla randevu oluşturuldu! Form validasyonu çalışmıyor!");
                    extentTest.fail("🐛 CRITICAL BUG: Sayfa içeriği - "
                            + (pageText.contains("Congratulations") ? "Congratulations mesajı tespit edildi!"
                                    : "Success mesajı var!"));
                } else {
